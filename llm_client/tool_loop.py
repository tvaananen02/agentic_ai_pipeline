...
# Some models write tool calls as text like <function(name)(...)></function>
# instead of using real tool_calls. Recover them if we see it.
_PSEUDO_CALL_PATTERN = re.compile(r"<function\((\w+)\)(?:=|\()(\{.*?\})\)?>\s*</function>", re.DOTALL)

# Cap resent history per tool result to avoid blowing tight TPM limits.
MAX_TOOL_RESULT_CHARS = 1500

# Only cache pure calls - write_file is a safe no-op to repeat.
...
async def _execute_tool_call(session, name: str, arguments: dict, call_cache: dict, log_fn) -> str:
    ...
    if name in _CACHEABLE_TOOLS and cache_key in call_cache:
